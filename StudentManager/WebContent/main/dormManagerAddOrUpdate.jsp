...
<div class="data_list">
		...
		<c:if test="${not empty user.id }">修改管理员</c:if>
				
		<c:if test="${empty user.id }">添加管理员</c:if>
		</div>
					...
					<table align="center">
						...
						<tr>
							...
							<c:forEach items="${builds }" var="build">
								<input name="dormBuildId" value="${build.id }"  style="heigth:14px;vertical-align:top"  type="checkbox" >${build.name }  &nbsp;
							</c:forEach>
							</td>
						</tr>
					</table>
